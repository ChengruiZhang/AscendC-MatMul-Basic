...
rm -rf *.vcd *.dump *.log *.bin *.o *.so *pu build output/*.bin
# 不需要TIK打印出内存信息
...
if [ -n "$8" ]; then
    rep=$8
    batch=$9
    echo "repeats: $8, batch: $9"
else
    echo "repeats: $8, batch: $9"
fi

# 生成计算输入数据和对比用的真值数据
# python3 $FILE_NAME.py $M $N $K
...
function compile_and_execute() {
    ...
    # 执行生成的可执行文件
    # (export LD_LIBRARY_PATH=`pwd`:$ASCEND_HOME_DIR/tools/simulator/${VersionMap[$SOC_VERSION]}/lib:$LD_LIBRARY_PATH && ./${1}_${4} "$M" "$N" "$K" "$rep" "$batch")
    (export LD_LIBRARY_PATH=`pwd`:$ASCEND_HOME_DIR/tools/simulator/${VersionMap[$SOC_VERSION]}/lib:$LD_LIBRARY_PATH )
    if [ $? -ne 0 ]; then
